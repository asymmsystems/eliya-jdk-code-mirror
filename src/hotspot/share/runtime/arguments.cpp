...
// Returned buffer is allocated from the C heap (mtArguments) and retained
// for the lifetime of the JVM via the flag's internal storage.
...
// command-line settings (FLAG_IS_CMDLINE silent-override per ADR-00001 §2.5
// tier 1).
//
...
// or unified-logging subsystem internals. Every FLAG_SET_ERGO-friendly
// capability is fully wired in this patch.
void Arguments::apply_eliya_production_profile() {
...
//     the FLAG_IS_CMDLINE guards in apply_eliya_production_profile() above
...
// Phase 2 / Phase 4 patches extend the body without changing the apply_ergo
// dispatch.
void Arguments::check_eliya_flag_consistency() {
...
jint Arguments::apply_ergo() {
  ...
  // Eliya — applied last so it observes the final state of other ergonomic
  // decisions, and so that explicit user command-line flags still win.
  // Per ADR-00001 §6.1, dispatch on the EliyaProfile enum value. The
  // EliyaProfileConstraintFunc (registered in globals.hpp) has already
  // rejected unrecognised and Phase-4-reserved values at parse time, so
  // here we only handle the currently-valid Phase 1 cases.
  if (EliyaProfile != nullptr && strcmp(EliyaProfile, "Production") == 0) {
    apply_eliya_production_profile();
  }
  if (EliyaConflictCheck) {
    check_eliya_flag_consistency();
  }
